...
export const token = `Bearer ${jwt.sign({ userId }, process.env.JWT_SECRET!, { expiresIn: process.env.JWT_EXPIRES_IN as jwt.SignOptions["expiresIn"] })}`;
export const refreshToken = jwt.sign({ userId }, process.env.JWT_REFRESH_SECRET!, { expiresIn: process.env.JWT_REFRESH_EXPIRES_IN as jwt.SignOptions["expiresIn"] });
export const getRefreshToken = (userid: mongoose.Types.ObjectId) => {
  return jwt.sign({ userId: userid }, process.env.JWT_REFRESH_SECRET!, { expiresIn: process.env.JWT_REFRESH_EXPIRES_IN as jwt.SignOptions["expiresIn"] });
};
...
beforeAll(async () => {
  const testUri = process.env.DATABASE_URL_TEST;
  if (!testUri) {
    throw new Error('DATABASE_URL_TEST is not defined in the .env file');
  }
